...
Standard module - workaround https://github.com/saltstack/salt/issues/10796 (nagios module needs to be able to change PLUGINDIR)
'''
...
def __virtual__():
    ...
    '''
    if os.path.isdir(_get_plugindir()):
        return 'nagios'
    return False
...
def _get_plugindir():
    '''
    Get host-specific nagios plugin directory
    '''
    if __grains__['os_family']=='Arch':
       return '/usr/share/nagios/libexec/'
...
def _execute_cmd(plugin, args='', run_type='cmd.retcode'):
    ...
    if plugin in all_plugins:
        data = __salt__[run_type](
                '{0}{1} {2}'.format(_get_plugindir(), plugin, args),
                python_shell=False)
...
def list_plugins():
    ...
    '''
    plugin_list = os.listdir(_get_plugindir())
    ret = []
    for plugin in plugin_list:
        # Check if execute bit
        stat_f = os.path.join(_get_plugindir(), plugin)
